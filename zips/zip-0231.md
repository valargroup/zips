...
been pruned (i.e. $\mathtt{fAllPruned} = 1$), memo decryption SHOULD NOT be
...
| † varies | $\mathtt{vMemoChunks}$ | $\mathtt{byte}[272][\mathtt{nMemoChunks}]$        | A sequence of encrypted memo chunks.                                   |
...
Nodes MUST reject `GetData` responses having an $\mathtt{fAllPruned}$ value that is
nonzero. A node that has pruned the memo bundle from a transaction MUST NOT serve
that transaction in response to a `GetData` request; it SHOULD respond as though
